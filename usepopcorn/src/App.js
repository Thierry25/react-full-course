...
export default function App() {
  ...
  function handleSelectedMovieId(id) {
    setSelectedId((mvId) => (mvId === id ? null : id));
  }
  ...
  function handleDeleteWatched(id) {
    setWatched((watched) => watched.filter((wM) => wM.imdbID !== id));
  }
...
function MovieDetails({
  ...
  const watchedIds = watched.map((wM) => wM.imdbID);
        ...
        <>
          ...
          <section>
            <div className="rating">
              {!watchedIds.includes(selectedId) ? (
                <>
                  <StarRating
                    maxRating={10}
                    size={24}
